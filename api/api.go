...
func NewApi(r *gin.Engine, cfg *config.Config, store storage.StorageI, logger logger.LoggerI) {
	...
	// user api
	r.POST("/user", handler.CreateCategory)
	r.GET("/user/:id", handler.GetByIdCategory)
	r.GET("/user", handler.GetListCategory)
	r.PUT("/user/:id", handler.UpdateCategory)
	r.DELETE("/user/:id", handler.DeleteCategory)
